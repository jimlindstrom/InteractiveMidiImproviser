...
class PitchGenerator
  def initialize
    ...
    interval_critic_order = 3
    @interval_critic = IntervalCritic.new(interval_critic_order)
    @critics.push @interval_critic

    pitch_and_pitch_class_set_critic_order = 6
    @pitch_and_pitch_class_set_critic = PitchAndPitchClassSetCritic.new(pitch_and_pitch_class_set_critic_order)

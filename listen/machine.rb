...
module Listen
	...
	class MachineContext
		...
		def respond
			if @response_lambda.nil?
				raise RuntimeError.new("cannot invoke response because lambda is nil")
			end
			if @machine.nil?
				raise RuntimeError.new("cannot invoke response because machine is nil")
			end

			@response_lambda.call(@event_queue, 
			                      lambda{@machine.response_done})
		end
	...
	def Listen::create_listener_machine(response_lambda)
		...
		listener_machine.context.set_response_lambda(response_lambda)

...
module Midi
	...
	# Inter-onset interval = the time delta between when two consecutive notes 
	# start.  An IOIArray is, well, an array of them.
	class IOIArray < Array
		...
		def quantize_helper(q1, q2, num_segs, max_depth)
			delta=(q2-q1)/(num_segs-1).to_f
			qs=[q1.to_f]
			(num_segs-1).times do
				qs.push(qs.last+delta)
			end
